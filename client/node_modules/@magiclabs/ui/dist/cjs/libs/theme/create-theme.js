...
function createRgbaColorResolvers(colorRepresentation) {
    const color = Color__default['default'](colorRepresentation);
    return {
        string: (alpha = 1) => {
...
function createTheme(config) {
    ...
    // Color values
    const primaryColor = Color__default['default'](typeof config.primaryColor === 'string' ? config.primaryColor : config.primaryColor.base);
    const lightColor = Color__default['default'](typeof config.lightColor === 'string' ? config.lightColor : config.lightColor.base);
    const darkColor = Color__default['default'](typeof config.darkColor === 'string' ? config.darkColor : config.darkColor.base);
    const midColor = Color__default['default'](typeof config.midColor === 'string' ? config.midColor : config.midColor.base);
    const successColor = Color__default['default'](typeof config.successColor === 'string' ? config.successColor : config.successColor.base);
    const warningColor = Color__default['default'](typeof config.warningColor === 'string' ? config.warningColor : config.warningColor.base);
    const errorColor = Color__default['default'](typeof config.errorColor === 'string' ? config.errorColor : config.errorColor.base);
    // Font values
    ...
    const createHexGroup = (base, def) => {
        const withDefaultSemantics = {
            base: base.hex(),
            lighter: typeof def === 'string' ? lightenBy(base, colorValueShift).hex() : Color__default['default'](def.lighter).hex(),
            lightest: typeof def === 'string' ? lightenBy(base, colorValueShiftFurther).hex() : Color__default['default'](def.lightest).hex(),
            darker: typeof def === 'string' ? darkenBy(base, colorValueShift).hex() : Color__default['default'](def.darker).hex(),
            darkest: typeof def === 'string' ? darkenBy(base, colorValueShiftFurther).hex() : Color__default['default'](def.darkest).hex(),
        };
        //   `darker` -> `lighter`
        // `lightest` -> `darkest`
        // ...and so forth
    ...
    const createColorGroup = (hex) => ({
        base: Color__default['default'](hex.base),
        lighter: Color__default['default'](hex.lighter),
        lightest: Color__default['default'](hex.lightest),
        darker: Color__default['default'](hex.darker),
        darkest: Color__default['default'](hex.darkest),
    });
    const colorResult = {
